...
function replaceText(word, replacement, text) {
  ...
  const regepx = new RegExp(word, "g");

  const replaceText = text.replace(regepx, replacement);
...
function checkWord(word, text) {
  ...
  const regepx = new RegExp(word, "i");

  const testSenten = regepx.test(text);
...
function extractTextInParentheses(str) {
  ...
  const check = str.matchAll(regepx);
...
function countEmails(str) {
  ...
  const regepx = /\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}\b/g;

  const check = str.match(regepx);
...
function findWordOccurrences(str, word) {
  ...
  // Повертаємо масив
  let regepx = new RegExp(word, "gi");

  let array = [];
  let match;

  while ((match = regepx.exec(str)) !== null) {
...
function checkRegexFlags(regex) {
  ...
  let check = allFlags.includes("g", "m");
...
function replaceWordOccurrences(str, word, newWord) {
  ...
  let regepx = new RegExp(word, "g");

  let replaceNew = str.replace(regepx, newWord);
...
function checkFlags(regex) {
  ...
  if (check.includes("i")) {
    newArray.push("ingoreCase");
  }
  let sourceCode = regex.source;

  newArray.push(sourceCode);
...
function checkRegexMethods(regex) {
  ...
  let checkDot = regex.dotAll;
  newArray.push("dotAll");
  let checkM = regex.multiline;
  newArray.push("multiline");
  let sticky = regex.sticky;
  newArray.push("sticky");
...
function findWord(str, word) {
  ...
  let regepx = new RegExp(word, "");

  let check = str.search(regepx);
